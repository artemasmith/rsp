...
RSpec.describe GameService do
  ...
  describe 'When run succesfully' do
    ...
    it { expect(service.call('rock')[:result]).to eq('win') }
    it { expect(service.call('rock')[:service_throw]).to eq(system_throw) }
  end
  ...
  describe 'When service throw unavailable' do
    ...
    it { expect(possible_results).to include(service.call('rock')[:result]) }
  end
end
